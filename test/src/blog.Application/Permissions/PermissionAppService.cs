...
namespace blog.Permissions
{
    public class PermissionAppService : ApplicationService, IPermissionAppService
    {
        ...
        //Update list permisison for a role
        ...
        //Update list permisison for a role
        public async Task UpdatePermissionAsync(UpdatePermissionOfRoleRequest input)
        {
            foreach (var permissionDto in input.updatedPermissionDto.Permissions)
            {
                await PermissionManager.SetAsync(permissionDto.Name, input.providerName, input.providerKey, permissionDto.IsGranted);
            }
        }

        //API: update a permission
        public async Task UpdateAPermissionAsync(UpdatePermissionRequest input)
        {
            await PermissionManager.SetAsync(input.PermissionName, input.ProviderName, input.ProviderKey, input.IsGranted);
        }
